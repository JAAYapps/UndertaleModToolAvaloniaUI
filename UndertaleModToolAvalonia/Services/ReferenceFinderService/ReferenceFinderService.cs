...
namespace UndertaleModToolAvalonia.Services.ReferenceFinderService
{
    ...
    public class ReferenceFinderService : IReferenceFinderService
    {
        ...
        private Dictionary<UndertaleCode, HashSet<UndertaleString>> stringReferences;
        private Dictionary<UndertaleCode, HashSet<UndertaleFunction>> funcReferences;
        ...
        public async Task<Dictionary<string, List<object>>?> GetReferencesOfObject(object obj, UndertaleData data, HashSetTypesOverride types, bool checkOne = false)
        {
            if (obj is null)
                return null;
        ...
        public async Task<Dictionary<string, List<object>>> GetUnreferencedObjects(UndertaleData data, Dictionary<Type, string> typesDict)
        {
            this.data = data;
            ...
            try
            {
                ...
                await loadingDialog.UpdateProgressAsync(0, assets.Count);
                await loadingDialog.UpdateStatusAsync("Assets");
                ...
                if (assets.Count > 0) // A Partitioner can't be created on an empty list.
                {
                    ...
                    await Task.Run(() =>
                    {
                        ...
                        Parallel.ForEach(assetsPart, (range) =>
                        {
                            ...
                            {
                                var asset = assets[i];
                                var assetReferences = GetReferencesOfObject(asset.Item1, data, new HashSetTypesOverride(true, data.Code is null), true);
                                ...
                                loadingDialog.UpdateProgressAsync(++count, assets.Count);
                            }
